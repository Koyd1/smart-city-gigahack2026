...
from app.core.civic_bundle import MAX_ARCHIVE_BYTES, load_civic_bundle
...
from app.queue import (
    ...
    enqueue_civic_bundle_import,
    enqueue_ingest,
    get_civic_bundle_import,
)
...
@router.post("/civic-bundle/import", status_code=status.HTTP_202_ACCEPTED)
async def create_civic_bundle_import(file: UploadFile = File(...)) -> dict[str, str | int]:
    filename = _sanitize_filename(file.filename or "bundle.zip")
    if Path(filename).suffix.lower() != ".zip":
        raise HTTPException(status_code=415, detail="Only ZIP civic bundles are supported")

    payload = await file.read(MAX_ARCHIVE_BYTES + 1)
    if not payload:
        raise HTTPException(status_code=400, detail="ZIP file is empty")
    if len(payload) > MAX_ARCHIVE_BYTES:
        raise HTTPException(status_code=413, detail=f"ZIP file is too large (max {MAX_ARCHIVE_BYTES} bytes)")
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Invalid civic bundle: {exc}") from exc

    job_id = str(uuid4())
    try:
        await enqueue_civic_bundle_import(job_id, filename, payload)
    except IngestQueueUnavailable as exc:
        raise HTTPException(status_code=503, detail="Civic bundle import queue is unavailable") from exc
...
@router.get("/civic-bundle/import/{job_id}")
async def get_civic_bundle_import_status(job_id: str) -> dict[str, object]:
    try:
        result = await get_civic_bundle_import(job_id)
    except IngestQueueUnavailable as exc:
        raise HTTPException(status_code=503, detail="Civic bundle import status is unavailable") from exc
    if result is None:
        raise HTTPException(status_code=404, detail="Civic bundle import not found")
    return result
